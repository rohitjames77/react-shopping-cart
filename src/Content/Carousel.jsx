...
function HeroCarousel({ slides }) {
  ...
  return (
    <div className="relative w-full h-[400px] md:h-[500px] overflow-hidden rounded-2xl">
      ...
      <button onClick={prevSlide} className="absolute left-4 top-1/2 -translate-y-1/2 bg-white/30 hover:bg-white/50 text-white p-3 rounded-full backdrop-blur-sm">
        ←
      </button>
      <button onClick={nextSlide} className="absolute right-4 top-1/2 -translate-y-1/2 bg-white/30 hover:bg-white/50 text-white p-3 rounded-full backdrop-blur-sm">
        →
      </button>
      ...
      {deals.map((deal) => (
        <div
          ...
          className={`bg-gradient-to-br ${
            deal.color === 'red' ? 'from-red-500 to-red-700' :
            deal.color === 'blue' ? 'from-blue-500 to-blue-700' :
...
function BrandsCarousel({ brands }) {
  ...
  return (
    <div className="flex flex-wrap justify-center items-center gap-8 md:gap-12">
      {brands.map((brand) => (
        <div key={brand.id} className="text-2xl font-bold text-gray-400 hover:text-gray-700 transition-colors cursor-pointer">
          {brand.name}
        </div>
      ))}
    </div>
