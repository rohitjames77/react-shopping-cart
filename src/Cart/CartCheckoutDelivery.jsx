...
export default function CartCheckoutDelivery() {
  ...
  const { selectedProductArr, quantity, setQuantity, setSelectedProductArr } =
    useOutletContext();
  ...
  return (
    ...
    >
      ...
      >
        <div
          className={`row-start-1 col-start-2 col-end-5 h-[35%] w-[70%] rounded-xl flex items-center gap-2 bg-green-100 border border-green-400 text-green-700 px-4 py-3 rounded animate-pulse delay-3 ${logInSuccess ? "block" : "hidden"} `}
        >
        ...
        <CartNavHeader />
        {couponNotification ? (
          <div className=" bg-green-100 text-green-700 pl-6 rounded border border-green-300 row-start-1 col-start-5 col-end-7 w-[75%] h-[30%] m-2 ">
            ✅ Coupon submitted successfully
          </div>
        ) : (
          <div className=" hidden  bg-green-100 text-green-700 pl-6 rounded border border-green-300 col-start-5 col-end-7 w-[75%] h-[30%] m-2">
            ✅ Coupon submitted successfully
          </div>
        )}
        {isPaymentProceeded ? (
          <Payment
            ...
            setIsVisaSelected={setIsVisaSelected}
          />
        ) : (
          <CartInventories
